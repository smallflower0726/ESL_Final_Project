...
class BasicOptions : public Options
{
    public:
        ...
        // Add
        addr_t sobelFilter_start_addr_0 = 0x73000000;
        addr_t sobelFilter_size_0 = 0x01000000;
        addr_t sobelFilter_end_addr_0 = sobelFilter_start_addr_0 + sobelFilter_size_0 - 1;
        addr_t sobelFilter_start_addr_1 = 0x76000000;
        addr_t sobelFilter_size_1 = 0x01000000;
        addr_t sobelFilter_end_addr_1 = sobelFilter_start_addr_1 + sobelFilter_size_1 - 1;
...
int sc_main(int argc, char **argv)
{
    ...
    ISS core_0(0, opt.use_E_base_isa);
    ISS core_1(1, opt.use_E_base_isa);
    ...
    CombinedMemoryInterface iss_mem_if_0("MemoryInterface_0", core_0);
    CombinedMemoryInterface iss_mem_if_1("MemoryInterface_1", core_1);
    ...
    // Add
    SobelFilter sobel_filter_0("sobel_filter_0");
    SobelFilter sobel_filter_1("sobel_filter_1");

    MemoryDMI dmi = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
    InstrMemoryProxy instr_mem_0(dmi, core_0);
    InstrMemoryProxy instr_mem_1(dmi, core_1);

    std::shared_ptr<BusLock> bus_lock = std::make_shared<BusLock>();
    iss_mem_if_0.bus_lock = bus_lock;
    iss_mem_if_1.bus_lock = bus_lock;

    instr_memory_if *instr_mem_if_0 = &iss_mem_if_0;
    data_memory_if *data_mem_if_0 = &iss_mem_if_0;
    instr_memory_if *instr_mem_if_1 = &iss_mem_if_1;
    data_memory_if *data_mem_if_1 = &iss_mem_if_1;
    ...
    if (opt.use_data_dmi)
    {
        iss_mem_if_0.dmi_ranges.emplace_back(dmi);
        iss_mem_if_1.dmi_ranges.emplace_back(dmi);
    }
     ...
     */
    core_0.init(instr_mem_if_0, data_mem_if_0, &clint, entry_point, rv64_align_address(opt.mem_end_addr));
    core_1.init(instr_mem_if_1, data_mem_if_1, &clint, entry_point, rv64_align_address(opt.mem_end_addr));
    sys.init(mem.data, opt.mem_start_addr, loader.get_heap_addr());
    sys.register_core(&core_0);
    sys.register_core(&core_1);
    ...
    core_0.error_on_zero_traphandler = opt.error_on_zero_traphandler;
    core_1.error_on_zero_traphandler = opt.error_on_zero_traphandler;
    ...
    // address mapping
    {
        ...
        // Add
        bus.ports[it++] = new PortMapping(opt.sobelFilter_start_addr_0, opt.sobelFilter_end_addr_0);
        bus.ports[it++] = new PortMapping(opt.sobelFilter_start_addr_1, opt.sobelFilter_end_addr_1);
    }

    // connect TLM sockets
    iss_mem_if_0.isock.bind(bus.tsocks[0]);
    iss_mem_if_1.isock.bind(bus.tsocks[3]);
...

    {
        ...
        // Add
        bus.isocks[it++].bind(sobel_filter_0.tsock);
        bus.isocks[it++].bind(sobel_filter_1.tsock);
    }

    // connect interrupt signals/communication
    plic.target_harts[0] = &core_0;
    plic.target_harts[1] = &core_1;
    clint.target_harts[0] = &core_0;
    clint.target_harts[1] = &core_1;
    sensor.plic = &plic;
    dma.plic = &plic;
    timer.plic = &plic;
    sensor2.plic = &plic;
    ...
    threads.push_back(&core_0);
    threads.push_back(&core_1);

    core_0.trace = opt.trace_mode; // switch for printing instructions
    core_1.trace = opt.trace_mode; // switch for printing instructions
    if (opt.use_debug_runner)
    {
        auto server = new GDBServer("GDBServer", threads, &dbg_if, opt.debug_port);
        new GDBServerRunner("GDBRunner", server, &core_0);
        new GDBServerRunner("GDBRunner", server, &core_1);
    }
    else
    {
        new DirectCoreRunner(core_0);
        new DirectCoreRunner(core_1);
    }
